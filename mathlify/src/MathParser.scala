...
object MathParser:
  ...
  def number(using P[Any]): P[MathExpr[Double]] =
  ...
  def braceContent(using P[Any]): P[MathExpr[Double]] = P("{" ~ expr ~ "}")

  def simpleAtom(using P[Any]): P[MathExpr[Double]] =
    P(number | identifier.map { name =>
      if knownConstants.contains(name) then Constant(name) else Symbol(name)
    })

  def braceOrAtom(using P[Any]): P[MathExpr[Double]] = P(braceContent | simpleAtom)

  def sumIndexBound(using P[Any]): P[(MathExpr[Double], MathExpr[Double])] =
    P("{" ~ expr ~ "=" ~ expr ~ "}")

  def args(using P[Any]): P[List[MathExpr[Double]]] =
    P(expr ~ ("," ~ expr).rep).map { case (h, t) => h :: t.toList }

  def sqrtExpr(using P[Any]): P[MathExpr[Double]] =
    P("sqrt" ~ "(" ~ expr ~ ")").map(arg => Root(None, arg))

  def sumExpr(using P[Any]): P[MathExpr[Double]] =
    P("sum_" ~ sumIndexBound ~ "^" ~ braceOrAtom ~ expr).map { case (idx, lo, hi, body) =>
      Sum(idx, lo, hi, body)
    }

  def integralExpr(using P[Any]): P[MathExpr[Double]] =
    P("int_" ~ braceOrAtom ~ "^" ~ braceOrAtom ~ expr).map { case (lo, hi, body) =>
      Integral(Symbol("x"), lo, hi, body)
    }

  def parenExpr(using P[Any]): P[MathExpr[Double]] =
    P("(" ~ expr ~ ")").map(Group(_))

  def funcCallOrSymbol(using P[Any]): P[MathExpr[Double]] =
  ...
  def atom(using P[Any]): P[MathExpr[Double]] =
    P(number | sqrtExpr | sumExpr | integralExpr | parenExpr | funcCallOrSymbol)

  def postfix(using P[Any]): P[MathExpr[Double]] =
    P(atom ~ ("_" ~ atom).?).map {
      case (base, Some(sub)) => Subscript(base, sub)
      case (base, None)      => base
    }

  def negExpr(using P[Any]): P[MathExpr[Double]] =
    P(("-" ~/ negExpr).map(Neg(_)) | postfix)

  def powExpr(using P[Any]): P[MathExpr[Double]] =
    P(negExpr ~ ("^" ~ negExpr).?).map {
      case (base, Some(exp)) => Pow(base, exp)
      case (base, None)      => base
    }

  def mulOp(using P[Any]): P[(String, MathExpr[Double])] =
    P(
      ("*" | "/").! ~ powExpr |
        (&(CharIn("a-zA-Z(")) ~ powExpr).map("*" -> _)
    )

  def mulExpr(using P[Any]): P[MathExpr[Double]] =
  ...
  def addExpr(using P[Any]): P[MathExpr[Double]] =
  ...
  def expr(using P[Any]): P[MathExpr[Double]] = P(addExpr)

  def parse(input: String): Either[String, MathExpr[Double]] =
